...
Public Class Assessment
    ...
    Public Sub ViewSchoolFeesForPayment(gv As DataGridView, gvid As DataGridView, btn1 As Button)
        Try
            Dim sql As String
            sql = "select assessedfeesid,feesname,feestype,assessedfees.amount,discounts,balance from feesschoolyear inner join fees on feesschoolyear.feesid = fees.feesid" _
                    & " inner join schoolyear on feesschoolyear.schoolyearid = schoolyear.schoolyearid" _
                    & " inner join gradelevel on feesschoolyear.gradelevelid = gradelevel.gradelevelid" _
                    & " inner join assessedfees on feesschoolyear.feessyid = assessedfees.feessyid" _
                    & " inner join enrollment on assessedfees.enrollmentid = enrollment.enrollmentid" _
            ...
            If IsConnected() = True Then
                ...
                gv.Rows.Add("", "", "", "", "", "", "", "")
                gv.Rows(0).Cells(0).Value = 0
                gv.Rows(0).Cells(1).Value = "Prev. Balance"
                gv.Rows(0).Cells(2).Value = "School Fees"
                gv.Rows(0).Cells(3).Value = Decimal.Parse(e.GetAmount()).ToString("N", CultureInfo.InvariantCulture)
                gv.Rows(0).Cells(4).Value = Decimal.Parse(e.GetDiscount()).ToString("N", CultureInfo.InvariantCulture)
                gv.Rows(0).Cells(5).Value = Decimal.Parse(gv.Rows(0).Cells(3).Value - gv.Rows(0).Cells(4).Value).ToString("N", CultureInfo.InvariantCulture)
                gv.Rows(0).Cells(6).Value = py.GetPayments(e.StudentID, sy.SchoolYearID).ToString("N", CultureInfo.InvariantCulture)
                gv.Rows(0).Cells(7).Value = Decimal.Parse(gv.Rows(0).Cells(5).Value - gv.Rows(0).Cells(6).Value).ToString("N", CultureInfo.InvariantCulture)
                ...
                For Each r As DataGridViewRow In gv.Rows
                    If r.Index > 0 Then
                        r.Cells(6).Value = py.GetSelectedPayment(r.Cells(0).Value).ToString("N", CultureInfo.InvariantCulture)
                    End If
                Next
    ...
    Public Sub ViewSchoolFeesToPay(gv As DataGridView, stud As Integer)
        Try
            ...
            If IsConnected() = True Then
                ...
                Dim Paymenttrack As Decimal = py.GetPayments(e.StudentID, sy.SchoolYearID)
        ...
        End Try
    End Sub
    Public Sub ViewSchoolFeesForPayment2(gv As DataGridView, id As Integer)
        Try
            Dim sql As String
            sql = "select assessedfeesid,feesname,feestype,assessedfees.amount,discounts,balance,schoolyearno,enrollment.enrollmentid from feesschoolyear inner join fees on feesschoolyear.feesid = fees.feesid" _
                    & " inner join schoolyear on feesschoolyear.schoolyearid = schoolyear.schoolyearid" _
                    & " inner join gradelevel on feesschoolyear.gradelevelid = gradelevel.gradelevelid" _
                    & " inner join assessedfees on feesschoolyear.feessyid = assessedfees.feessyid" _
                    & " inner join enrollment on assessedfees.enrollmentid = enrollment.enrollmentid" _
                    & " where enrollment.studentid = " & id & " and astatus = 'Yes'  ORDER BY schoolyearno,feestype,feesname ASC;"
            If IsConnected() = True Then
                gv.Rows.Clear()
                Dim py As New Payments
                Dim e As New Enrollment
                ...
                While reader.Read()
                    ...
                    gv.Rows.Add(reader("assessedfeesid"), reader("feesname"), reader("feestype"), _
                        ...
                        Decimal.Parse(reader("discounts")).ToString("N", CultureInfo.InvariantCulture),
                        Decimal.Parse(reader("balance")).ToString("N", CultureInfo.InvariantCulture),
                        pd.ToString("N", CultureInfo.InvariantCulture), reader("schoolyearno"),
                        pd.ToString("N", CultureInfo.InvariantCulture), False, reader("enrollmentid"))
                End While
                reader.Close()
                gv.ClearSelection()
                ...
                'Tracking of Payment
                For Each r As DataGridViewRow In gv.Rows
                    py.EnrollmentID = r.Cells(10).Value
                    r.Cells(6).Value = py.GetSelectedPayment(r.Cells(0).Value).ToString("N", CultureInfo.InvariantCulture)
                Next
                '

                For Each r As DataGridViewRow In gv.Rows
                    r.Cells(8).Value = Decimal.Parse(r.Cells(5).Value - r.Cells(6).Value).ToString("N", CultureInfo.InvariantCulture)
                Next
                RemoveOtherRow(gv)
                gv.ClearSelection()
                gv.FirstDisplayedScrollingRowIndex = gv.RowCount - 1
    ...
    Sub RemoveOtherRow(gv As DataGridView)
        For Each r As DataGridViewRow In gv.rows
            If Decimal.Parse(r.Cells(8).Value) <= 0 Then
                gv.Rows.Remove(r)
                RemoveOtherRow(gv)
            End If
        Next
